    ...
    <div className="space-y-6">
      <div className="bg-[#0b0c10]/40 p-4 rounded-3xl border border-zinc-800/40 space-y-4 shadow-xs">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
          {[
            { id: "all", label: "Всі", icon: Filter, count: stats.all, color: "text-zinc-400" },
            { id: "pending", label: "Очікують", icon: Clock, count: stats.pending, color: "text-amber-500" },
            { id: "paid", label: "Оплачені", icon: CheckCircle2, count: stats.paid, color: "text-emerald-500" },
            { id: "shipped", label: "В дорозі", icon: Truck, count: stats.shipped, color: "text-violet-500" },
            { id: "delivered", label: "Доставлені", icon: Package, count: stats.delivered, color: "text-emerald-500" },
            { id: "cancelled", label: "Скасовані", icon: XCircle, count: stats.cancelled, color: "text-red-500" },
          ].map((f) => (
            <button
              key={f.id}
            ...
            >
              ...
              <span className={`text-[10px] font-black ${activeFilter === f.id ? "text-violet-400/90" : "text-zinc-600"}`}>
                {f.count}
              </span>
            </button>
          ))}
      ...
      <div className="flex flex-col gap-4 pb-20">
        {filteredOrders.length > 0 ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {order.items.slice(0, 3).map((item, idx) => (
                        ...
                        >
                          {item.name}{" "}
                          <span className="text-violet-400 ml-1">x{item.quantity}</span>
                        </div>
                      ))}
          ...
          <div className="py-20 text-center bg-zinc-900/5 border border-dashed border-zinc-800/40 rounded-3xl">
            <p className="text-zinc-500 text-sm italic">Список замовлень порожній</p>
          </div>
        )}
      </div>
    </div>
